...
export async function routeLeg(a,b,mode='walking',map=null,region=''){const B=await sdk();return new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(Error('路线查询超时，请稍后重试。')),12000);const C=mode==='driving'?B.DrivingRoute:B.WalkingRoute;const route=new C(a.point,{...(map?{renderOptions:{map,autoViewport:true}}:{}),onSearchComplete:r=>{clearTimeout(timer);if(route.getStatus()!==0||!r?.getNumPlans())return reject(Error('未找到可用路线，请更换出行方式或地点。'));const p=r.getPlan(0);const path=[];for(let i=0;i<p.getNumRoutes();i++)for(const point of p.getRoute(i).getPath())path.push({lng:point.lng,lat:point.lat});resolve({seconds:p.getDuration(false),meters:p.getDistance(false),from:a.name,to:b.name,path,navigation:navigationUrl(a,b,mode,region),queriedAt:new Date().toISOString()})}});route.search(a.point,b.point)})}
export async function planTrip(items,places,mode){const points=[],regions=[];for(const i of items){const p=places.find(x=>x.id===i.id);points.push(await resolvePlace(p));regions.push(p.region)}const legs=[];for(let i=1;i<points.length;i++)legs.push(await routeLeg(points[i-1],points[i],mode,null,regions[i-1]));return legs;}
