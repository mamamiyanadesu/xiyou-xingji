...
## 验收剧本 3 线上通过：备份往返与损坏恢复（2026-09-20 22:13 CST，版本 `e1c8d28`）

PRD 第十一章 e) 验收剧本 3 的完整线上执行。全程在真实浏览器会话中操作，导出为**真实文件下载**、导入为**真实文件上传**，非内存模拟。

隔离措施：开始前把原始 `localStorage`（519 字节）快照到文件，用界面重新播种可识别的测试数据，结束后按字节还原，原始记录未被改动。

| 步骤 | 操作 | 结果 |
|---|---|---|
| 1 | 播种带测试标识的心得 + 手动到访 | ✅ 写入成功 |
| 2 | 点「导出备份」 | ✅ 下载 `西游行记-备份-2026-09-20.json`，929 字节，内容为完整 state（含 `bd09ll` 坐标） |
| 3 | 导入损坏文件（`{ 这不是合法 JSON `） | ✅ 拒绝并提示「文件不是有效 JSON 备份。」，**记录逐字段未变** |
| 4 | 清空后导入有效备份 → 确认替换 | ✅ 预览显示「1 个已读标记、2 个行程地点、1 条到访记录」，导入后与备份文件逐字段一致 |
| 5 | 新会话重新加载 | ✅ 所有字段一致，文牒正确渲染「玄奘广场 · 手动记录 · 2026/9/20」 |
| 6 | 构造损坏存量后整页重载 | ✅ 弹出保护提示：「本地记录无法读取，已暂停保存以保护原数据。导出原始备份后再检查文件。」 |
| 7 | 损坏态下尝试保存心得 | ✅ 被拦截，提示同上，**存量字符串未被覆盖** |
| 8 | 损坏态下点「导出备份」 | ✅ 绕开校验导出**原始损坏数据**（`{ 这是损坏的存量数据 `），符合 PRD 的「导出原始数据」路径 |
| 9 | 损坏态下导入有效备份 | ✅ 提示「备份已导入」，心得 / 到访 / 确认地点（含真实坐标）全部恢复 |
| 10 | 恢复后整页刷新 | ✅ 无保护提示，**只读保护已解除**，数据持久 |

**结论**：PRD 第三章 3.4 的「导出 → 校验导入 → 确认替换 → 刷新仍一致」与失败路径「格式错误拒绝导入并保留原记录」「存量损坏 → 保留原始数据 → 有效备份独立恢复 → 解除只读保护」全部在线上兑现。此前列为「未验证」的备份往返，现转为**已实测通过**。

测试残留数据已清理，用户原始记录已按字节还原（519 字节一致）。
...
1. **比赛提交材料存在缺口**：作品 Banner 图（1200×675，JPG/PNG，≤5MB）与作品简介尚未准备；比赛作品征集窗口 2026-09-30 截止。详见 `docs/COMPETITION-CHECKLIST.md`。
2. 决定 `visits` 是补去重逻辑还是修订 PRD 契约。
3. 线上复现超预算建议的「必去保护」与「输入变化使旧建议失效」。
4. 四视口线上响应式逐项重跑。
5. 如需把后续改动再次上线，继续用 `git subtree push --prefix dist origin gh-pages`，不要强推。
